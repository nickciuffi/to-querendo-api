...
public class IntencaoCompraController {
    ...
    @GetMapping("/banhistas")
    @VendedorOnly
    @Operation(
            summary = "Consultar banhistas com intenção de compra compatível",
            description = "Retorna os turistas (banhistas) da mesma praia do vendedor autenticado que possuem intenção "
                    + "de compra ativa para algum dos produtos ativos do vendedor. Para cada banhista, somente as "
                    + "intenções compatíveis com os produtos do vendedor são listadas. Restrito a usuários com a role de vendedor."
    )
    @ApiResponses(value = {
            @io.swagger.v3.oas.annotations.responses.ApiResponse(
                    responseCode = "200",
                    description = "Banhistas consultados com sucesso"
            ),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(
                    responseCode = "401",
                    description = "Token ausente, inválido ou expirado",
